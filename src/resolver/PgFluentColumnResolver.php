...
class PgFluentColumnResolver extends BaseColumnResolver
{
    ...
    {
        [$type, $size, $default, $nullable, $comment] = $this->resolveCommon($column);

        return $this->buildString([$type . $size, $nullable, $default, $comment]);
    }

    /**
     * @param \yii\db\ColumnSchema|\yii\db\pgsql\ColumnSchema $column
     * @return string
     */
    protected function resolveCommon(ColumnSchema $column)
    {
        ...
        if (isset($intMap[$type]) && $column->dimension === 0) {
            $type = $intMap[$type];
        }
        ...
        $comment =
            $column->comment ? ("comment(" . $this->schema->quoteValue($column->comment) . ")")
                : '';
    ...
    protected function buildDefaultValue(ColumnSchema $column)
    {
        ...
        switch (gettype($column->defaultValue)) {
            ...
            case 'double':
                ...
                $string =
                    'defaultValue("' . str_replace(',', '.', (string)$column->defaultValue) . '")';
                break;
            case 'boolean':
    ...
    {
        $columnParts = array_filter($columnParts,
            function($v) {
                return $v !== '!skip';
            });
    ...
    {
        [$type, $size, , , $comment] = $this->resolveCommon($column);
    ...
    protected function resolveJsonType(ColumnSchema $column)
    {
        ...
        } elseif ($column->defaultValue instanceof JsonExpression) {
            $default = "DEFAULT '" . json_encode($column->defaultValue->getValue()) . "'";
    ...
    protected function resolveArrayType(ColumnSchema $column)
    {
        ...
        if ($column->allowNull === true && $column->defaultValue === null) {
            ...
            $default = $column->allowNull === true ? ' DEFAULT NULL' : '';
        } elseif ($column->defaultValue instanceof ArrayExpression) {
            $default = 'DEFAULT \'{' . implode(', ', $column->defaultValue->getValue()) . '}\'';
