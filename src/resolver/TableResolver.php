...
class TableResolver implements IMigrationTableResolver
{
    ...
    public function getIndexes($tableName)
    {
        ...
        if ($this->connection->driverName == 'mysql') {
            $query = $this->connection->createCommand('SHOW INDEX FROM [[' . $tableName . ']]')
                                      ->queryAll();
            if ($query) {
                foreach ($query as $i => $index) {
                    $indexes[$index['Key_name']]['cols'][$index['Seq_in_index']] =
                        trim($index['Column_name'], '\'"');
                    $indexes[$index['Key_name']]['isuniq'] =
                        ($index['Non_unique'] == 1) ? false : true;
                }
            }
            ...
            if (!empty($schemaIndexes)) {
                foreach ($schemaIndexes as $i => $columns) {
                    if (!$columns['ispk']) {
                        $indexes[$columns['indexname']]['cols'][] =
                            trim($columns['columnname'], '\'"');
                        $indexes[$columns['indexname']]['isuniq'] =
                            $columns['isuniq'] ? true : false;
                    }
                }
            }
            ...
            if (!empty($schemaIndexes)) {
                foreach ($schemaIndexes as $indexName => $columns) {
                    $indexes[$indexName]['cols'] =
                        array_walk($columns, function(&$v) { $v = trim($v, '\'"'); });
