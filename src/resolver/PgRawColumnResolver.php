...
class PgRawColumnResolver extends BaseColumnResolver
{
    ...
    {
        [$type, $size, $default, $nullable] = $this->resolveCommon($column);
    ...
    protected function buildDefaultValue(ColumnSchema $column)
    {
        ...
        if ($column->defaultValue instanceof ArrayExpression) {
            return $string . "'{" . implode(', ', $column->defaultValue->getValue()) . "}'";
        }
    ...
    protected function buildString(array $columnParts)
    {
        ...
        return (!empty($type) ? $type : '') . (!empty($columnParts) || $size ? '."' . ($size ? $size
                    . ' ' : ' ') . $columnParts . '"' : '');
    }
    ...
    {
        [$type, , ,] = $this->resolveCommon($column);
