...
class RawColumnResolver extends BaseColumnResolver
{
    ...
    protected function resolveCommon(ColumnSchema $column)
    {
        ...
        $nullable = $column->allowNull === true ? 'NULL' : 'NOT NULL';
        $comment =
    ...
    protected function buildString(array $columnParts)
    {
        ...
        return (!empty($type) ? $type : '') . (!empty($columnParts) ? '."' . ($size ? $size . ' '
                    : ' ') . $columnParts
                . '"' : '');
    }
    ...
    {
        [, , $default, $nullable, $comment] = $this->resolveCommon($column);
        if ($column->enumValues) {
            $enum = "enum(" . implode(', ',
                    array_map([$this->schema, 'quoteValue'], $column->enumValues)) . ")";
        } else {
            return "";
        }
        $columns =
            implode(' ', array_filter(array_map('trim', [$nullable, $default, $comment]), 'trim'));
        return '"' . $enum . ' ' . $columns . '"';
    }
    ...
    protected function resolveNumeric(ColumnSchema $column)
    {
        ...
        if (count($pk) === 1 && in_array($column->name, $pk)) {
            if ($column->unsigned) {
                $column->type =
                    ($column->type == Schema::TYPE_BIGINT ? Schema::TYPE_UBIGPK : Schema::TYPE_UPK);
            } else {
                $column->type =
                    ($column->type == Schema::TYPE_BIGINT ? Schema::TYPE_BIGPK : Schema::TYPE_PK);
            }
    ...
    {
        [$type, , , , $comment] = $this->resolveCommon($column);
