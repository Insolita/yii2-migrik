...
class ByModelGenerator extends Generator
{
    ...
    protected function prepareColumns($modelInfo, $phpdocInfo)
    {
        ...
        if (!$this->phpdocOnly && !empty($modelInfo['columns'])) {
            foreach ($modelInfo['columns'] as $column) {
                ...
                $comment =
                    isset($modelInfo['labels'][$column]) ? $modelInfo['labels'][$column] : '';
    ...
    protected function getInfoFromModel($model)
    {
         ...
         **/
        $modelResolver =
            Yii::createObject(['class' => 'insolita\migrik\contracts\IModelResolver'], [$model]);
        $info = [
    ...
    protected function getInfoFromPhpdoc($model)
    {
         ...
         **/
        $modelResolver =
            Yii::createObject(['class' => 'insolita\migrik\contracts\IPhpdocResolver'], [$model]);
        $info = [
    ...
    protected function validateModelClass($modelClass)
    {
        try {
            if (!class_exists($modelClass)) {
                $this->addError('models',
                    "Class '$modelClass' does not exist or has syntax error.");
                return false;
            }
        } catch (Exception $e) {
