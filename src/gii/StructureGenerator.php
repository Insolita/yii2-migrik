...
class StructureGenerator extends Generator
{
    ...
    public function validateTableName()
    {
        ...
        if (empty($tables)) {
            $this->addError('tableName',
                "Table '{$this->tableName}' does not exist, or all tables was ignored");
            return false;
        }
        return true;
    }
    ...
    protected function checkIfPkColumnIsInteger($pk, $tableColumns)
    {
        ...
        if ($this->format === self::FORMAT_FLUENT) {
            if (mb_strpos($pkColumn, 'primaryKey') !== false
                || mb_strpos($pkColumn,
                    'bigPrimaryKey') !== false) {
                $check = true;
            }
    ...
    protected function createColumnResolver(TableSchema $tableSchema)
    {
        ...
        if ($this->resolverClass) {
            ...
            switch ($this->getDbConnection()->driverName) {
                case 'pgsql':
                {
                    return Yii::createObject(['class' => 'insolita\migrik\resolver\PgFluentColumnResolver'],
                        $params);
                }
                case 'mysql':
                default:
                {
                    return Yii::createObject(['class' => 'insolita\migrik\resolver\FluentColumnResolver'],
                        $params);
                }
            }
        } else {
            switch ($this->getDbConnection()->driverName) {
                case 'pgsql':
                {
                    return Yii::createObject(['class' => 'insolita\migrik\resolver\PgRawColumnResolver'],
                        $params);
                }
                case 'mysql':
                default:
                {
                    return Yii::createObject(['class' => 'insolita\migrik\resolver\RawColumnResolver'],
                        $params);
                }
            }
        }
